...
neg_plot <- ggplot(tree.time, aes(x = yr, fill = factor(neg_inc_annual))) +
    ...
    theme_minimal() +
    labs(
        title = "Number of trees with negative increment values each year",
        x = "Year",
        y = "Number of trees"
    ) +
...
neg_species_plot <- ggplot(neg_species, aes(x = Species, y = inc_annual, fill = factor(yr))) +
    ...
    theme_minimal() +
    labs(
        title = "Proportion of trees with negative increment values in 2010 and 2015",
        x = "Species",
...
twi_plot <- ggplot(tree.time %>% filter(yr %in% c(2010, 2015)), aes(x = twi, y = neg_inc_annual)) +
    ...
    facet_wrap(~yr) +
    labs(
        title = "Relationship between TWI and negative increment values",
        x = "TWI",
...
size_plot <- ggplot(tree.time %>% filter(yr %in% c(2010, 2015)), aes(x = calcDBH_min1, y = neg_inc_annual)) +
    ...
    facet_wrap(~yr) +
    labs(
        title = "Relationship between size and negative increment values",
        x = "Size",
        y = "Negative increment values"
    ) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
