...
resistance_top4 <- top4_noouts %>%
  ...
  filter(!is.infinite(resistance.div))%>%
         ...
         dplyr::mutate(
         quantiles_div=cut(resistance.div,
         breaks=quantile(resistance.div, 
         #probs=c(0.25, 0.5, 0.75, 1), 
         probs=seq(0, 1, 0.25),
         na.rm=T),
         labels=c("1", "2", "3", "4")))
...
resistance_map<-ggplot(resistance_top4, aes(x=X, y=Y, size=as.numeric(DBH.04), col=quantiles_div))+
  geom_point(alpha=0.5)+
