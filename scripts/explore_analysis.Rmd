...
spagplot_top4<-ggplot() +
  #species plots
  geom_line(data=top4 %>% group_by(Cno, sp) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc, 
  group=factor(sp,levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI")), 
  col=factor(sp,levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI"))))+
  geom_line(data=top4 %>% group_by(Cno) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc), col="black", size=2)+
...
spagplot_top10<-ggplot()+
  #species plots
  geom_line(data=dendro_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno, sp) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc, 
  group=sp, col=sp))+
  #mean of all trees
  geom_line(data=dendro_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc), col="black", size=2)+
  #mean of species
  geom_line(data=dendro_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno, sp) %>% 
  dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)) %>%
  ungroup()%>%
  group_by(Cno) %>% dplyr::summarise(mean_inc=mean(mean_inc, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc), col="grey40", size=0.8)+
  scale_color_viridis_d()+
...
spagplot_all<-ggplot() +
  #species plots
  geom_line(data=dendro_inc %>% group_by(Cno, sp) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc, 
  group=sp, col=sp))+
  #mean of all trees
  geom_line(data=dendro_inc %>% group_by(Cno) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=mean_inc), col="black", size=2)+
  #mean of species
