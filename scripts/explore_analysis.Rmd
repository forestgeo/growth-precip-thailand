...
pre_yr_census<-ggplot()+
  ...
  xlab("")+theme_bw()+ ylab("total monthly precipitation (mm)")+
...
dendro_2010<- dendro.data.noouts %>%
  ...
  mutate(calcDBH_min1=dplyr::lag(calcDBH, n=1, default=NA, order_by=Tag),
         ...
         inc.time=year(Date2)-lag(year(Date2)),
...
resistance_2010<-merge(resistance_2010, dec_williams, by="sp", all.x=T)
...
resistance_2015<-merge(resistance_2015, dec_williams, by="sp", all.x=T)
...
png("../doc/display/resistance_dif.png", width=8, height=4, units="in", res=300)
grid.arrange(resistance_2010_dif, resistance_2015_dif, nrow=1)
dev.off()
...
resistance_model<-resistance_all %>%
  select(resistance.dif, calcDBH_min1, cii_min1, williams_dec, yr)
