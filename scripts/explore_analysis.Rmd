...
spei_hkk<-read.csv("data/climate/SPEI_HKK_from_GEE.csv")
...
```
```{r forest fire station, echo=F}

rainfall_hkk<-read.csv("data/climate/WeatherData_ALL_forest_fire_research_station.csv")
head(rainfall_hkk)
max(rainfall_hkk$YearII)
...
```{r top 4 resistance and exposure, echo=F}

#exposure vs resistance plot

resistance_exposure<-ggplot(resistance_top4, aes(x=cii_min1, y=resistance.div, 
col=factor(sp, levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI"))))+
  geom_boxplot()+
  geom_smooth(method="lm", se=F)+
  facet_grid(sp~yr)+
  scale_color_viridis_d()+
  xlab("exposure")+ylab("resistance")+theme_bw()

resistance_exposure

```

```{r associated datasets, echo=F}
...
ggplot(habitat, aes(x=x, y=y, fill=factor(hab)))+
  geom_tile()+
...
```
```{r TWI from elev, echo=F}
#elevation
#although .rdata, this needs to be read using read.table - thanks Bier!

hkk_elev<-read.table("data/HKK-other/CTFSElev_hkk.rdata")

png("growth-precip-thailand/doc/display/elevation_resistance_map_2015.png", width=8, height=4, units="in", res=300)
ggplot(hkk_elev, aes(x=col.x, y=col.y, fill=col.elev))+
geom_tile()+
scale_fill_viridis_c(option="E")+
#add contours
geom_contour(aes(z=col.elev), colour="black")+
geom_point(data=resistance_clean %>% filter(yr==2015, sp %in% c("HOPEOD", "TETRNU", "POLYVI", "SACCLI")), 
aes(x=X.x, y=Y, size=as.numeric(DBH.04)), inherit.aes=F, col="black", alpha=0.5)+
...
elev_rast<-raster(elev_mat, xmn=0, xmx=50, ymn=0, ymx=100,
...
wbt_hillshade(dem = "data/HKK-other/HKKDEMsm_5m.tif",
...
wbt_d_inf_flow_accumulation(input = "data/HKK-other/HKKDEMsm_filled_breached.tif",
                            output = "data/HKK-other/HKK_DinfFAsca.tif",
                            out_type = "Specific Contributing Area")

wbt_slope(dem = "data/HKK-other/HKKDEMsm_filled_breached.tif",
          output = "data/HKK-other/demslope.tif",
          units = "degrees")

wbt_wetness_index(sca = "data/HKK-other/HKK_DinfFAsca.tif",
...
```{r crowding variables, echo=F}

#find basal area around each tree
...
#for each tree in each census, find the basal area of trees within 5, 10, 15 and 20m
library(sf)
#first convert to sf object
hkk.stem1_sf<-st_as_sf(hkk.stem1, coords=c("gx", "gy"))
hkk.stem2_sf<-st_as_sf(hkk.stem2, coords=c("gx", "gy"))
hkk.stem3_sf<-st_as_sf(hkk.stem3, coords=c("gx", "gy"))
hkk.stem4_sf<-st_as_sf(hkk.stem4, coords=c("gx", "gy"))

#then find the distance between each tree and all other trees
hkk.stem1_dist<-st_distance(hkk.stem1_sf)
...
resistance <- dendro_inc_clean %>%
  ...
  filter(!is.infinite(resistance.div))%>%
         ...
         #remove trees that have -ve long-term increment
          filter(avg_inc>0)
...
resistance_prop<-ggplot(data = resistance, aes(x=resistance.prop)) +
  geom_density(alpha=0.5) +
  facet_grid(.~yr)+
  xlim(-5, 5)+
  xlab("proportion of change from long-term mean")+
  ggtitle("resistance values for all trees")+
  geom_vline(xintercept = c(-1, 0,1), lty=2)+
  theme_bw()

png("growth-precip-thailand/doc/display/resistance_prop.png", width=6, height=4, units="in", res=300)
resistance_prop
dev.off()
...
```{r resistance with predictor variables, echo=F}

resistance_clean <- resistance %>%
filter(abs(resistance.div)<5)

sum(resistance$resistance.div>5, na.rm=T)/nrow(resistance)
...
png("growth-precip-thailand/doc/display/resistance_bysp.png", width=8, height=6, units="in", res=300)
ggplot(data=resistance_clean %>% filter(sp %in% sp_top10), aes(x=factor(sp), y=resistance.prop, fill=factor(sp)))+ 
  geom_violin()+
  geom_boxplot(fill="white", width=0.1)+
  geom_hline(yintercept=c(-1, 0, 1), lty=2)+
  facet_wrap(~yr)+
  ylab("growth decrease from long term mean")+
  #make ylab percentage
...
ggplot()+
  geom_point(data=resistance_clean, aes(x=calcDBH_min1, 
  y=resistance.prop))+
  geom_smooth(data=resistance_clean, aes(x=calcDBH_min1, 
  y=resistance.prop), method="lm")+
  facet_grid(yr~.)+
  theme_bw()

#resistance with cii

ggplot(data=resistance_clean, aes(x=factor(cii_min1), fill=factor(cii_min1),
  #y=resistance.div))+
  y=resistance.prop))+
  geom_violin()+
  geom_boxplot(fill="white", width=0.1)+
...
#resistance with deciduousness

ggplot()+
  geom_point(data=resistance_clean, aes(x=williams_dec, 
  #y=resistance.div))+
  y=resistance.prop))+
  geom_smooth(data=resistance_clean, aes(x=williams_dec, 
  #y=resistance.div), method="lm")+
  y=resistance.prop), method="lm")+
  facet_grid(yr~.)+
  theme_bw()

#resistance with habitat

ggplot(data=resistance_clean, aes(x=factor(hab), 
  y=resistance.div, fill=factor(hab)))+
  geom_violin()+
  geom_boxplot(fill="white", width=0.1)+
  scale_fill_viridis_d(option="E", direction=-1)+
  facet_grid(yr~.)+
...
resistance$ciiF<-ordered(resistance$cii_min1)
resistance$hab<-factor(resistance$hab)

model_2010<-glm(resistance.div ~ calcDBH_min1 + ciiF + 
williams_dec+hab+
                  calcDBH_min1*ciiF + 
                  calcDBH_min1*williams_dec+
                  ciiF*williams_dec,
                data=resistance[resistance$yr==2010,])

summary(model_2010)

model_2015<-glm(resistance.div ~ calcDBH_min1 + ciiF + 
williams_dec+hab+
                  calcDBH_min1*ciiF + 
                  calcDBH_min1*williams_dec+
                  ciiF*williams_dec,
                data=resistance[resistance$yr==2015,])
...
resistance_model<-resistance %>%
  select(resistance.div, calcDBH_min1, ciiF, williams_dec, hab, yr, quantiles_div)
...
library(lme4)
library(nlme)
model_all<-lm(resistance.div ~ calcDBH_min1 + ciiF + hab+ williams_dec+
                  calcDBH_min1*ciiF + 
                  calcDBH_min1*williams_dec+
...
library(dagitty)
library(ggdag)
growth.dag<-dagitty( 'dag {
bb="0,0,1,1"
crowding [pos="0.427,0.650"]
exposure [exposure,pos="0.405,0.325"]
growth [outcome,pos="0.583,0.335"]
size [exposure,pos="0.409,0.151"]
topography [exposure,pos="0.259,0.454"]
water [latent,pos="0.415,0.471"]
exposure -> growth
size -> exposure
topography -> water
water -> crowding
...
png("growth-precip-thailand/doc/display/dag_ind.png", width=4, height=4, units="in", res=300)
ggdag_status(growth.dag, text = FALSE, use_labels = "name") +
  guides(color = FALSE) +  # Turn off legend
  theme_dag()
dev.off()

impliedConditionalIndependencies( growth.dag )
```

```{r species DAG, echo=F}

sp.dag<-dagitty('dag {
bb="0,0,1,1"
"canopy position" [exposure,pos="0.276,0.255"]
"median size" [exposure,pos="0.440,0.108"]
deciduousness [exposure,pos="0.458,0.618"]
exposure [latent,pos="0.458,0.238"]
growth [outcome,pos="0.609,0.329"]
water [latent,pos="0.449,0.427"]
"canopy position" -> exposure
"median size" -> exposure
deciduousness -> water
exposure -> growth
water -> growth
}')

png("growth-precip-thailand/doc/display/dag_sp.png", width=4, height=4, units="in", res=300)
ggdag_status(sp.dag, text = FALSE, use_labels = "name") +
  guides(color = FALSE) +  # Turn off legend
  theme_dag()
dev.off()

impliedConditionalIndependencies( sp.dag )
