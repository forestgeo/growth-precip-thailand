...
```
```{r calculating increments for specific years}
...
library(gridExtra)
png("../doc/display/increments_raw.png", width=8, height=4, units="in", res=300)
...
hkk_census <- hkk_census %>%
  filter(status=="A")%>%
  filter(tag %in% dendro.data.noouts$Tag)%>%
  group_by(StemTag)%>%
  dplyr::mutate(dbhmin1 = dplyr::lag(dbh, n=1, default=NA, order_by=StemTag),
                inc_annual = (dbh-dbhmin1)/5)%>%
  filter(!is.na(dbhmin1))%>%
  dplyr::summarise(avg_inc = mean(inc_annual, na.rm=T),
                   sd_inc = sd(inc_annual, na.rm=T))
...
png("../doc/display/census_increment.png", width=4, height=4, units="in", res=300)
inc_census
dev.off()
...
resistance_all<-rbind(resistance_2010, resistance_2015, resistance_2019)
resistance_all$yr<-c(rep(2010, nrow(resistance_2010)), rep(2015, nrow(resistance_2015)), rep(2019, nrow(resistance_2019)))
...
png("../doc/display/resistance_map.png", width=8, height=10, units="in", res=300)
resistance_map
dev.off()
```
```{r basic model}

model_2010<-glm(resistance.dif ~ calcDBH_min1 + cii_min1 + williams_dec+
                  calcDBH_min1*cii_min1 + 
                  calcDBH_min1*williams_dec+
                  cii_min1*williams_dec,
                data=resistance_2010)

summary(model_2010)

model_2015<-glm(resistance.dif ~ calcDBH_min1 + cii_min1 + williams_dec+
                  calcDBH_min1*cii_min1 + 
                  calcDBH_min1*williams_dec+
                  cii_min1*williams_dec,
                data=resistance_2015)

summary(model_2015)

model_2019<-glm(resistance.dif ~ calcDBH_min1 + cii_min1 + williams_dec+
                  calcDBH_min1*cii_min1 + 
                  calcDBH_min1*williams_dec+
                  cii_min1*williams_dec,
                data=resistance_2019)
...
resistance_model<-resistance_all %>%
  select(resistance.dif, calcDBH_min1, cii_min1, williams_dec, yr, quantile)
...
library(lme4)
library(nlme)
model_all<-lme(resistance.dif ~ calcDBH_min1 + cii_min1 + williams_dec+
                  calcDBH_min1*cii_min1 + 
                  calcDBH_min1*williams_dec+
                  cii_min1*williams_dec,
                random=~1|yr,
...
plot_model(model_all, type="pred", terms=c("calcDBH_min1","williams_dec"), show.data = T)
