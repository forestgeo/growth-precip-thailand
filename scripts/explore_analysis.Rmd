...
saccli_plot<-ggplot() +
  geom_line(data = saccli, aes(x=((Cno-3)/2)+2009, y=inc_annual, group=Tag), alpha=0.5) +
  ...
  geom_line(data=saccli %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=median_inc), col="blue")+
  xlab("year") + ylab("diameter increment (cm)")+
  ggtitle("growth increments for SACCLI (deciduous)")+
  theme_bw()

hopeod_plot<-ggplot() +
  geom_line(data = hopeod, aes(x=((Cno-3)/2)+2009, y=inc_annual, group=Tag), alpha=0.5) +
  ...
  geom_line(data=hopeod %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=median_inc), col="blue")+
  xlab("year") + ylab("diameter increment (cm)")+
  ggtitle("growth increments for HOPEOD (evergreen)")+
  theme_bw()

tetrnu_plot<-ggplot() +
  geom_line(data = tetrnu, aes(x=((Cno-3)/2)+2009, y=inc_annual, group=Tag), alpha=0.5) +
  ...
  geom_line(data=tetrnu %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=median_inc), col="blue")+
  xlab("year") + ylab("diameter increment (cm)")+
  ggtitle("growth increments for TETRNU (deciduous)")+
  theme_bw()

polyvi_plot<-ggplot() +
  geom_line(data = polyvi, aes(x=((Cno-3)/2)+2009, y=inc_annual, group=Tag), alpha=0.5) +
  ...
  geom_line(data=polyvi %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=median_inc), col="blue")+
  xlab("year") + ylab("diameter increment (cm)")+
...
spagplot_top4_dbh<-ggplot() +
  #species plots
  geom_line(data=top4_dbh %>% group_by(Cno, sp) %>% 
  dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=median_inc, 
  group=factor(sp,levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI")), 
  col=factor(sp,levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI"))))+
  geom_line(data=top4_dbh %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=median_inc), col="black", size=2)+
  scale_color_viridis_d()+
  guides(col=guide_legend(title="species"))+
  xlab("year") + ylab("diameter increment (cm)")+
...
spagplot_dbh_ind<-ggplot() +
  geom_line(data = top4_dbh, aes(x=((Cno-3)/2)+2009, y=inc_annual, group=Tag), alpha=0.5) +
  geom_line(data=top4_dbh %>% group_by(Cno) %>% dplyr::summarise(mean_inc=mean(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=mean_inc), col="red")+
  geom_line(data=top4_dbh %>% group_by(Cno) %>% dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), aes(x=((Cno-3)/2)+2009, y=median_inc), col="blue")+
  facet_wrap(factor(sp, levels=c("HOPEOD", "TETRNU", "POLYVI", "SACCLI"))~., 
  nrow=2)+
  xlab("year") + ylab("diameter increment (cm)")+
  ggtitle("growth increments for top 4 species from DBH measurements")+
  theme_bw()

png("growth-precip-thailand/doc/display/spaghetti_top4_dbh_ind.png", width=8, height=8, units="in", res=300)
spagplot_dbh_ind
dev.off()

png("growth-precip-thailand/doc/display/spaghetti_top4_dbh.png", width=4, height=4, units="in", res=300)
spagplot_top4_dbh
dev.off()
...
spagplot_top10_dbh<-ggplot()+
  #species plots
  geom_line(data=dbh_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno, sp) %>% 
  dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=median_inc, 
  group=sp, col=sp))+
  #mean of all trees
  geom_line(data=dbh_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno) %>% 
  dplyr::summarise(median_inc=median(inc_annual, na.rm=T)), 
  aes(x=((Cno-3)/2)+2009, y=median_inc), col="black", size=2)+
  #mean of species
  geom_line(data=dbh_inc %>% filter(sp %in% sp_top10)%>% group_by(Cno, sp) %>% 
...
png("growth-precip-thailand/doc/display/spaghetti_top10_dbh.png", width=4, height=4, units="in", res=300)
spagplot_top10_dbh
dev.off()
...
```
```{r comparing dbh and dendro, echo=F}

#merge dbh and dendro data by Tag and Cno

inc_all<-merge(dendro_inc, dbh_inc, by=c("Tag", "Cno"), all=T)

#plot increments from dbh and dendro data

png("growth-precip-thailand/doc/display/dbh_dendro_compare.png", width=4, height=4, units="in", res=300)
