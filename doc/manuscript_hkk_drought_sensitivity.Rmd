...
Finally, we calculated the anomaly for each variable on each day of 2010 and 2015 from the long-term expectation as $$anomaly = \frac{rolling.mean - rolling.mean_{long-term}}{rolling.mean{sd}}$$   
...
To account for potential biases in annual increments from dendrobands from water-related growth or shrinkage, we also calculated growth occurrence using a zero-growth assumption [@zweifel_are_2016]. 
...
We extracted draws, calculated distributions using functions from the _posterior_ package [@vehtari_ranknormalization_2021; @bürkner_posterior_2023].
