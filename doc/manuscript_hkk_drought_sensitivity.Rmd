...
![***Figure `r fig_drought_responses`: Growth increment timeseries and drought sensitivity*** across trees and species in the Huai Kha Khaeng ForestGEO plot. a) Median of annualised growth increments across species (colours) and the whole plot (black) from 2009 - 2022 from late wet/early dry season dendroband measurements. 2010 and 2015 were moderate and very strong ENSO events respectively. b) Distribution of drought sensitivity of tree growth across all individuals in the two drought years. For comments - https://github.com/forestgeo/growth-precip-thailand/issues/11](display/Fig2.png)
...
Trees on average had negative drought sensitivities for both droughts with considerable variation around it (Fig `r fig_drought_responses`b). 
...
Deciduousness was associated with lower negative sensitivities in 2015, but not in 2010 (Fig `r fig_species`a). 
...
The opposite effects of TWI on sensitivity in the wet and dry season drought, and the influence of deciduousness was also confirmed by models that controlled for crown exposure and size as well as species variation in intercept and slopes (Fig `r fig_microenvs`a). 
...
Predictions across the range of observed variables also showed that deciduous species generally had larger negative sensitivities in 2015, which did not show increase with wetness (Fig `r fig_microenvs`c). 
...
Controlling for this direct effect of exposure, the remaining effect of size *per se* was significantly positive in 2010, while it was weakly negative in 2015 (Fig `r fig_microenvs`a, Fig `r fig_dag`a). 
...
Analysing two droughts in a 14-year dendrometer band timeseries, we found that tropical tree species and individuals grew less than average in drought years, reflecting in lower plot-level growth (Fig `r fig_drought_responses`). 
...
Interestingly, in the wet-season drought, we also show that evergreen and deciduous species had divergent responses along a wetness gradient; evergreen trees fared better and deciduous species suffered worse declines in wetter sites (Fig `r fig_species`b, Fig `r fig_microenvs`b). 
Further, besides consistent negative effects of crown exposure, we found that the drivers of variation among species and individuals in drought sensitivity were dissimilar between the drought events (Fig `r fig_microenvs`a). 
Water availability buffered exposure-associated growth declines through different mechanisms in the two different droughts - larger trees (with likely deeper roots) in the dry-season drought and trees in wetter sites in the wet-season drought suffered smaller declines in growth (Fig `r fig_dag`). 
