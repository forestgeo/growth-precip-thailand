...
tree.time.sp <- tree.time %>%
  group_by(Species) %>%
  dplyr::summarise(inc_annual_med = round(median(inc_annual, na.rm = T), 2)) %>%
...
Median annualised growth rate across the years of measurement for the whole plot was `r round(median(tree.time$inc_annual, na.rm=T),2)` cm. 
Median annual increment differed by species with `r tree.time.sp$Species[1]` having the lowest (`r tree.time.sp$inc_annual_med[1]` cm) and `r tree.time.sp$Species[nrow(tree.time.sp)]` the highest (`r tree.time.sp$inc_annual_med[nrow(tree.time.sp)]` cm)among the `r nrow(tree.time.sp)` species analysed. 
